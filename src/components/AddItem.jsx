...
// Tjekker magic bytes ‚Äî afviser alt der ikke er JPEG, PNG eller WebP
...
// Skalerer og komprimerer til maks 1920px + 2MB JPEG ‚Äî hurtigere AI og billigere upload
...
export default function AddItem({ onBack, onSaved }) {
  ...
  async function handleGenerateVinted() {
    ...
    if (isNaN(priceNum) || priceNum < 1 || priceNum > 10000) {
      setError('Pris skal vÊre mellem 1 og 10.000 kr')
      return
    }
    setLoading(true)
    setError('')
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Altid .jpg ‚Äî filnavnet er irrelevant for sikkerheden
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <button onClick={onBack} style={styles.backBtn}>‚Üê Tilbage</button>
            ...
            {imagePreview ? (
              <div>
                ...
                <button onClick={handleAnalyse} style={styles.primaryBtn} disabled={loading}>
                  {loading ? '‚è≥ Analyserer...' : '‚ú® AnalysÈr med AI'}
                </button>
              </div>
            ) : (
              <div style={styles.uploadArea} onClick={() => fileRef.current.click()}>
                ...
                <p style={styles.uploadHint}>eller vÊlg fra galleri</p>
              </div>
            )}
        ...
        {step === 'detaljer' && aiData && (
          <div style={styles.section}>
            ...
            <div style={styles.aiBox}>
              <h3 style={styles.aiTitle}>AI har gÊttet:</h3>
              <div style={styles.aiGrid}>
                <AiField label="MÊrke" value={aiData.brand} />
            ...
            <button onClick={handleGenerateVinted} style={styles.primaryBtn} disabled={loading}>
              {loading ? '‚è≥ Genererer opslag...' : '‚úçÔ∏è Lav Vinted-opslag'}
            </button>
          </div>
        )}
        ...
        {step === 'vinted' && (
          <div style={styles.section}>
            ...
            <button onClick={handleCopy} style={styles.copyBtn}>
              {copied ? '‚úÖ Kopieret!' : 'üìã KopiÈr opslag'}
            </button>
            {error && <p style={styles.error}>{error}</p>}
            <button onClick={handleSave} style={styles.primaryBtn} disabled={loading}>
              {loading ? '‚è≥ Gemmer...' : 'üíæ Gem i katalog'}
            </button>
          </div>
        )}
      </div>
